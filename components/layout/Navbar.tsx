        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex justify-between items-center w-full">
          ...
          <div className="flex items-center gap-4 md:gap-8 relative z-[60]">
            ...
            <button 
              ...
              className="text-[10px] md:text-xs font-semibold tracking-widest text-ash hover:text-white transition-colors border border-glass px-3 py-1.5 rounded-md shrink-0 bg-white/[0.02] backdrop-blur-sm"
            >
              {lang === "en" ? "العربية" : "EN"}
            </button>
            ...
            <div className="hidden md:flex items-center gap-8">
              ...
              {/* THE NEW VIP BESPOKE CONFIGURATOR BUTTON */}
              <Link 
                href="/configurator" 
              ...
              {!isLoading && (
                <>
                  {user ? (
                    ...
                    <Link href="/auth" className="text-xs font-medium tracking-widest uppercase text-white border-b border-crimson pb-1 ml-4 hover:text-crimson transition-colors">
                      {t("nav.client_portal")}
                    </Link>
                  )}
                </>
      ...
      <div className={`fixed inset-0 bg-obsidian/95 backdrop-blur-2xl z-50 flex flex-col items-center justify-center transition-all duration-500 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col items-center gap-8 w-full px-6">
          ...
          >
            ...
            <span className={`font-cinzel text-xl md:text-2xl text-center z-10 ${lang === 'ar' ? 'font-cairo font-bold tracking-normal' : 'tracking-widest'}`}>
              {t("home.vip_title")}
            </span>
          </Link>
          ...
          {!isLoading && user ? (
            ...
            <Link href="/auth" onClick={toggleMenu} className={`w-full max-w-xs bg-white text-obsidian py-4 rounded-xl uppercase tracking-[0.2em] text-xs font-bold text-center transition-all duration-500 delay-300 ${menuOpen ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}>
              {t("nav.client_portal")}
            </Link>
          )}
        </div>
