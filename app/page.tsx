    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 w-full max-w-5xl mt-20">
          <h1 className={`font-cinzel text-5xl md:text-8xl tracking-[0.05em] leading-tight mb-8 animate-[fadeInUp_1s_forwards] ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>
            {t("home.title_1")}<br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-crimson to-red-900">{t("home.title_2")}</span>
          </h1>
          <p className="font-light text-base md:text-xl text-ash max-w-2xl mx-auto leading-relaxed mb-12 opacity-0 animate-[fadeInUp_1s_0.2s_forwards]">
            {t("home.subtitle")}
          </p>
      ...
      <section id="studio" className="py-24 md:py-32 px-6 md:px-12 max-w-[1600px] mx-auto w-full relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-20 reveal-on-scroll opacity-0 translate-y-10 transition-all duration-1000 ease-luxury">
          <div>
            <h2 className={`font-cinzel text-3xl md:text-4xl mb-4 ${lang === 'ar' ? 'font-cairo font-semibold' : ''}`}>{t("home.select_marque")}</h2>
            <p className="text-ash font-light text-base md:text-lg">{t("home.choose_mfg")}</p>
          </div>
        </div>
            ...
            {brands.map((brand) => (
              ...
              >
                ...
                <div className="relative z-10 text-center transform group-hover:-translate-y-4 transition-transform duration-500 ease-luxury">
                  ...
                  <span className={`font-cinzel text-xl tracking-[0.3em] text-ash group-hover:text-white transition-colors duration-500 ${lang === 'ar' ? 'font-cairo font-bold tracking-normal' : ''}`}>
