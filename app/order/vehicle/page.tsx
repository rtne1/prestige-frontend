...
function OrderVehicleContent() {
  ...
  // Configuration Options (Using JSON Translations)
  const qtyOptions = [
  ...
  const oemOptions = [
    { id: "N", label: "N0/N1/N2 (Porsche)", desc: t("configurator.oem_approved_for") + " Porsche" },
    { id: "MO", label: "MO / MO1 (Mercedes)", desc: t("configurator.oem_approved_for") + " Mercedes" },
    { id: "BMW", label: "★ (BMW)", desc: t("configurator.oem_approved_for") + " BMW" },
    { id: "L", label: "L (Lamborghini)", desc: t("configurator.oem_approved_for") + " Lamborghini" },
    { id: "K", label: "K1 / K2 (Ferrari)", desc: t("configurator.oem_approved_for") + " Ferrari" },
    { id: "AM", label: "AM (Aston Martin)", desc: t("configurator.oem_approved_for") + " Aston Martin" },
    { id: "B", label: "B (Bentley)", desc: t("configurator.oem_approved_for") + " Bentley" },
    { id: "LR", label: "LR (Land Rover)", desc: t("configurator.oem_approved_for") + " Land Rover" },
  ];
        ...
        <div className="bg-gradient-to-b from-carbon/80 to-obsidian/90 backdrop-blur-3xl border border-white/10 p-8 lg:p-10 rounded-[2rem] lg:sticky lg:top-32 flex flex-col shadow-[0_30px_60px_rgba(0,0,0,0.8)] overflow-hidden relative group">
          ...
          <div className="relative z-10 flex flex-col items-center border-b border-white/10 pb-8 mb-8">
            {tireImg ? (
              <div className="relative w-56 lg:w-64 h-56 lg:h-64 flex items-center justify-center mb-6">
               ...
               <div className="w-56 h-56 bg-white/5 rounded-full mb-6 flex items-center justify-center text-ash/30 text-xs font-semibold uppercase tracking-widest border border-white/10">{t("order.no_image")}</div>
            )}
          ...
          <div className="relative z-10 flex-1">
            <div className="flex items-center gap-3 mb-6">
                ...
                <h4 className={`text-[10px] uppercase tracking-widest text-ash ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("configurator.specs")}</h4>
        ...
        <div className="space-y-6 text-start flex flex-col">
            ...
            {[1, 2].map((num) => (
                <div key={num} className={`flex flex-col gap-2 w-1/2 ${num === 2 ? (lang === 'ar' ? 'items-end' : 'items-end') : (lang === 'ar' ? 'items-start' : 'items-start')}`}>
                    ...
                    <span className={`text-[10px] uppercase tracking-widest ${step >= num ? 'text-white' : 'text-ash/50'} ${lang === 'ar' ? 'font-cairo' : ''}`}>
                        {num === 1 ? t("order.step_1_title") : t("order.step_2_title")}
                    </span>
                </div>
            ))}
          </div>
          ...
          <div className={`bg-carbon/60 backdrop-blur-xl border rounded-[2rem] p-6 md:p-10 transition-all duration-700 ${step === 1 ? 'border-white/20 shadow-[0_10px_50px_rgba(0,0,0,0.5)]' : 'border-white/5 opacity-50 hover:opacity-80 cursor-pointer'}`} onClick={() => step > 1 && setStep(1)}>
            <div className="flex justify-between items-center mb-8 gap-4">
              <h2 className={`font-cinzel text-2xl md:text-3xl text-white flex items-center gap-4 ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>
                ...
                {t("order.step_1_title")}
              </h2>
              {step > 1 && <button className={`text-[10px] uppercase tracking-widest text-crimson hover:text-white ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("order.edit")}</button>}
            </div>
                ...
                {tab === "search" && (
                  <div className="relative">
                    <div className="relative flex items-center">
                        ...
                        <input type="text" placeholder={t("order.search_placeholder")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full bg-black/40 border border-white/10 rounded-2xl py-5 text-base text-white outline-none focus:border-crimson focus:bg-black/60 transition-all shadow-inner ${lang === 'ar' ? 'pr-14 pl-12 font-cairo' : 'pl-14 pr-12'}`} />
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                        <select onChange={(e) => handleBrandChange(Number(e.target.value))} className={`w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-5 text-sm text-white outline-none focus:border-crimson appearance-none cursor-pointer transition-colors ${lang === 'ar' ? 'font-cairo' : ''}`}>
                            <option value="" className="text-black bg-white">{t("order.select_brand")}</option>
                    ...
                    {selBrand && (
                      <div className="relative animate-[fadeIn_0.3s_ease-out]">
                        <select onChange={(e) => handleModelChange(Number(e.target.value))} className={`w-full bg-black/40 border border-white/10 rounded-2xl px-6 py-5 text-sm text-white outline-none focus:border-crimson appearance-none cursor-pointer transition-colors ${lang === 'ar' ? 'font-cairo' : ''}`}>
                            <option value="" className="text-black bg-white">{t("order.select_model")}</option>
                    ...
                    {selModel && years.length > 0 && (
                      <div className="pt-4 animate-[fadeInUp_0.4s_ease-out]">
                        <span className={`text-[10px] uppercase tracking-widest text-ash mb-4 block px-2 ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("configurator.select_year")}</span>
              ...
              <div className="animate-[fadeInUp_0.5s_ease-out]">
                ...
                <div className="flex items-center justify-between bg-crimson/10 border border-crimson/30 rounded-2xl p-4 mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <span className={`text-[10px] text-crimson uppercase tracking-widest block font-bold mb-1 ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("order.selected_vehicle")}</span>
                ...
                {selectedVehicle.oemSpec && (
                  <div className="bg-gradient-to-br from-black/80 to-carbon border border-white/10 rounded-2xl p-6 md:p-8 mb-10 shadow-inner flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group hover:border-white/20 transition-colors">
                    ...
                    <div className="flex-1 w-full text-center md:text-start">
                      <span className={`text-[10px] uppercase tracking-[0.2em] text-ash/60 mb-3 block ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("configurator.front_axle")}</span>
                    ...
                    <div className="flex-1 w-full text-center md:text-start">
                      <span className={`text-[10px] uppercase tracking-[0.2em] text-ash/60 mb-3 block ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("configurator.rear_axle")}</span>
                      <div className="font-cinzel text-3xl md:text-4xl text-white font-bold tracking-wider">
                ...
                {/* OEM Mark Selection */}
                <h3 className={`text-[10px] uppercase tracking-widest text-ash mb-4 px-2 ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("order.oem_mark_title")}</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
                  {oemOptions.map(opt => (
                    <button key={opt.id} onClick={() => setSelectedOem(opt.id === selectedOem ? "" : opt.id)} className={`p-4 rounded-xl border text-start transition-all duration-300 flex flex-col justify-center gap-1 ${selectedOem === opt.id ? 'bg-crimson border-crimson text-white shadow-[0_10px_20px_rgba(204,0,0,0.3)] -translate-y-1' : 'bg-black/40 border-white/5 text-ash hover:border-white/20 hover:bg-white/5'}`}>
                      ...
                      <span className={`text-[9px] opacity-70 uppercase tracking-wider ${lang === 'ar' ? 'font-cairo' : ''}`}>{opt.desc}</span>
                    </button>
                  ))}
                </div>
          ...
          {step === 3 && (
              <div className="bg-carbon/60 backdrop-blur-xl border border-green-500/30 rounded-[2rem] p-10 flex flex-col items-center justify-center text-center animate-[fadeIn_0.5s_ease-out] shadow-[0_0_50px_rgba(34,197,94,0.1)]">
                  ...
                  <h3 className={`text-2xl font-cinzel text-white mb-2 ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>
                      {lang === 'ar' ? 'تمت إضافة الطلب' : 'Configuration Added'}
                  </h3>
                  <p className={`text-ash text-sm ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("cart.add_success")}</p>
              </div>
          )}
...
// Ensure Suspense loader is also localized dynamically
export default function Page() {
  const [lang, setLang] = useState('en');
  useEffect(() => { setLang(localStorage.getItem('language') || 'en'); }, []);
  ...
  return (
    <Suspense fallback={
        <div className="min-h-screen w-full bg-obsidian flex flex-col items-center justify-center gap-6">
            ...
            <span className={`text-ash uppercase tracking-widest text-xs ${lang === 'ar' ? 'font-cairo' : ''}`}>
