...
function OrderVehicleContent() {
  ...
  const [step, setStep] = useState<1 | 2 | 3>(1); // 1: Vehicle, 2: Qty, 3: Final Review
  ...
  const qtyOptions = [
    { id: "all", label: "Set of 4" },
    { id: "front_2", label: "Front (2 Tires)" },
    { id: "rear_2", label: "Rear (2 Tires)" },
    { id: "front_1", label: "Front (1 Tire)" },
    { id: "rear_1", label: "Rear (1 Tire)" },
  ];
  ...
  const handleModelChange = (id: number) => {
    ...
    api.get(`/vehicles/models/${id}/years`).then(res => {
      // Map it to match the VehicleResult interface
  ...
  const selectVehicle = (v: VehicleResult) => {
    ...
    setStep(2); // Move to Qty Step
  };

  const handleWhatsApp = () => {
    if (!tire || !selectedVehicle || !tireQty) return;
    const qtyLabel = qtyOptions.find(o => o.id === tireQty)?.label;
    const text = `Hello Mr. Tires, I would like to order:\n\n*Vehicle:* ${selectedVehicle.year} ${selectedVehicle.model.brand.name} ${selectedVehicle.model.name}\n*Tire:* ${tire.brand.name} ${tire.model_name}\n*Quantity:* ${qtyLabel}\n\n*Notes:* ${notes}`;
  ...
  const handleContinueToVault = async () => {
    ...
    try {
      const qtyLabel = qtyOptions.find(o => o.id === tireQty)?.label;
      const combinedNotes = `[Quantity: ${qtyLabel}]\n${notes}`.trim();
      ...
      if (user) {
        ...
        await api.post("/garage/requests", {
          ...
          compound_id: tire?.id,
          ...selectedVehicle?.oemSpec, // Send OEM specs if they exist
          client_notes: combinedNotes || null
        });
        router.push("/garage");
      } else {
        // Save pending data and push to auth
  ...
  return (
    <div className="min-h-screen bg-obsidian text-white pt-[100px] pb-32">
      <div className="max-w-[1200px] mx-auto w-full px-6 md:px-12 grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-12 items-start">
        ...
        {/* LEFT COLUMN: Tire Summary (Sticky) */}
        <div className="bg-carbon/50 border border-white/5 p-8 rounded-3xl lg:sticky lg:top-32 flex flex-col items-center text-center shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          {tireImg ? (
            <img src={tireImg} alt={tire?.model_name} className="w-48 h-auto drop-shadow-[0_20px_20px_rgba(0,0,0,0.8)] mb-6" />
             ...
             <div className="w-48 h-48 bg-white/5 rounded-full mb-6 flex items-center justify-center text-ash/30 text-xs">NO IMAGE</div>
          )}
          <span className="text-[10px] uppercase tracking-[0.2em] text-ash mb-2">{tire?.brand.name}</span>
          ...
          {selectedVehicle && (
            <div className="mt-8 pt-8 border-t border-white/10 w-full text-start animate-[fadeInUp_0.4s_ease-out]">
              <span className="text-[10px] uppercase tracking-widest text-ash mb-1 block">Selected Vehicle</span>
        ...
        <div className="space-y-6">
          ...
          <div className={`bg-carbon border rounded-3xl p-6 md:p-10 transition-all duration-500 ${step === 1 ? 'border-white/20 shadow-[0_0_30px_rgba(255,255,255,0.05)]' : 'border-white/5 opacity-60'}`}>
            <div className="flex justify-between items-center mb-8">
              <h2 className={`font-cinzel text-2xl ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>1. Identify Your Vehicle</h2>
              {step > 1 && <button onClick={() => setStep(1)} className="text-xs uppercase tracking-widest text-crimson hover:text-white">Edit</button>}
            </div>
            ...
            {step === 1 && (
              <div className="animate-[fadeInUp_0.3s_ease-out]">
                ...
                <div className="flex p-1 bg-obsidian rounded-xl mb-8 border border-white/5">
                  <button onClick={() => setTab("search")} className={`flex-1 py-3 text-xs uppercase tracking-widest rounded-lg transition-all ${tab === "search" ? 'bg-carbon text-white shadow-sm border border-white/10' : 'text-ash hover:text-white'}`}>Smart Search</button>
                  <button onClick={() => setTab("manual")} className={`flex-1 py-3 text-xs uppercase tracking-widest rounded-lg transition-all ${tab === "manual" ? 'bg-carbon text-white shadow-sm border border-white/10' : 'text-ash hover:text-white'}`}>Manual Select</button>
                </div>
                ...
                {tab === "search" && (
                  <div className="relative">
                    <input 
                      type="text" 
                      placeholder="e.g. Camry 2023, BMW X5..."
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full bg-obsidian border border-white/10 rounded-xl px-6 py-4 text-sm text-white outline-none focus:border-crimson transition-all"
                    />
                    {isSearching && <div className="absolute top-1/2 -translate-y-1/2 right-6 w-4 h-4 border-2 border-glass border-t-crimson rounded-full animate-spin"></div>}
                      ...
                      <div className="mt-4 bg-obsidian border border-white/5 rounded-xl overflow-hidden max-h-60 overflow-y-auto">
                        {searchResults.map(v => (
                          <button key={v.id} onClick={() => selectVehicle(v)} className="w-full text-start px-6 py-4 text-sm border-b border-white/5 hover:bg-carbon hover:text-crimson transition-colors last:border-0 flex justify-between items-center group">
                            ...
                            <svg className="w-4 h-4 text-ash group-hover:text-crimson opacity-0 group-hover:opacity-100 transition-all rtl:-scale-x-100" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                          </button>
                        ))}
                      </div>
                    )}
                ...
                {tab === "manual" && (
                  <div className="space-y-4">
                    <select onChange={(e) => handleBrandChange(Number(e.target.value))} className="w-full bg-obsidian border border-white/10 rounded-xl px-6 py-4 text-sm text-white outline-none focus:border-crimson appearance-none">
                      <option value="">Select Brand</option>
                      {brands.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                    </select>
                    {selBrand && (
                      <select onChange={(e) => handleModelChange(Number(e.target.value))} className="w-full bg-obsidian border border-white/10 rounded-xl px-6 py-4 text-sm text-white outline-none focus:border-crimson appearance-none animate-[fadeInUp_0.3s_ease-out]">
          ...
          <div className={`bg-carbon border rounded-3xl p-6 md:p-10 transition-all duration-500 ${step === 2 ? 'border-white/20 shadow-[0_0_30px_rgba(255,255,255,0.05)]' : 'border-white/5 opacity-60 pointer-events-none'}`}>
            <div className="flex justify-between items-center mb-8">
              <h2 className={`font-cinzel text-2xl ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>2. Fitment & Details</h2>
              {step > 2 && <button onClick={() => setStep(2)} className="text-xs uppercase tracking-widest text-crimson hover:text-white pointer-events-auto">Edit</button>}
            </div>
            ...
            {step === 2 && (
              <div className="animate-[fadeInUp_0.3s_ease-out]">
                <div className="flex flex-wrap gap-3 mb-8">
                  {qtyOptions.map(opt => (
                    <button key={opt.id} onClick={() => setTireQty(opt.id)} className={`px-5 py-3 rounded-xl border text-xs transition-all ${tireQty === opt.id ? 'bg-white border-white text-obsidian font-semibold shadow-[0_0_15px_rgba(255,255,255,0.2)]' : 'bg-obsidian border-white/10 text-ash hover:border-white hover:text-white'}`}>
                      {opt.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="mb-8">
                  <label className="text-[10px] uppercase tracking-widest text-ash mb-3 block px-2">Special Notes (Optional)</label>
                  <textarea 
                    className="w-full bg-obsidian border border-white/10 outline-none transition-all text-white p-5 rounded-xl focus:border-crimson min-h-[120px] resize-none text-sm placeholder:text-ash/30" 
                    placeholder="Enter any specific requests, questions about sizing, etc."
                    value={notes} 
                ...
                <button 
                  onClick={() => setStep(3)} 
                  disabled={!tireQty}
                  className="w-full bg-white text-obsidian px-6 py-4 rounded-xl uppercase tracking-widest text-xs font-bold hover:bg-crimson hover:text-white transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  Continue to Review
                </button>
              </div>
          ...
          <div className={`bg-carbon border rounded-3xl p-6 md:p-10 transition-all duration-500 ${step === 3 ? 'border-crimson/50 shadow-[0_0_40px_rgba(204,0,0,0.15)]' : 'border-white/5 opacity-60 pointer-events-none hidden md:block'}`}>
            <h2 className={`font-cinzel text-2xl mb-8 ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>3. Finalize Order</h2>

            {step === 3 && (
              <div className="animate-[fadeInUp_0.3s_ease-out]">
                <div className="flex flex-col sm:flex-row gap-4">
                  {/* WhatsApp Ordering */}
                  <button onClick={handleWhatsApp} className="flex-1 bg-[#25D366] text-obsidian px-6 py-5 rounded-xl uppercase tracking-widest text-xs font-bold hover:bg-[#20bd5a] transition-all flex flex-col items-center justify-center gap-2 shadow-[0_0_20px_rgba(37,211,102,0.2)]">
                  ...
                  <button onClick={handleContinueToVault} disabled={isSubmitting} className="flex-1 bg-crimson text-white px-6 py-5 rounded-xl uppercase tracking-widest text-[10px] font-bold hover:bg-white hover:text-obsidian transition-all shadow-[0_0_30px_rgba(204,0,0,0.3)] disabled:opacity-50 flex flex-col items-center justify-center gap-2">
                    ...
                    <span>{isSubmitting ? "Processing..." : (user ? "Submit to Dashboard" : "Login & Submit")}</span>
                  </button>
                </div>
                <p className="text-center text-ash text-[10px] tracking-widest uppercase mt-6">
                  Our concierge team will verify your vehicle's OEM fitment before finalizing the order.
                </p>
              </div>
            )}
          </div>
