      ...
      <div className="max-w-[1400px] mx-auto w-full px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 relative z-10">
        ...
        <div className="flex flex-col items-center justify-center relative animate-[fadeInUp_0.6s_ease-out]">
          <div className="relative w-full aspect-square md:aspect-[4/3] lg:aspect-square bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-carbon/80 to-obsidian border border-white/5 rounded-3xl overflow-hidden flex items-center justify-center p-8 group">
            {imageUrl ? (
              <img src={imageUrl} alt={tire.model_name} className="max-h-full max-w-full object-contain drop-shadow-[0_30px_30px_rgba(0,0,0,0.8)] transition-transform duration-[2s] ease-luxury group-hover:scale-110 relative z-10" />
            ) : (
        ...
        <div className="flex flex-col justify-center animate-[fadeInUp_0.8s_ease-out]">
          <div className="mb-8 border-b border-white/10 pb-8 text-start">
            <span className="text-xs uppercase tracking-[0.3em] text-crimson font-semibold mb-3 block">{tire.brand.name}</span>
            <h1 className={`font-cinzel text-4xl md:text-5xl lg:text-6xl text-white leading-tight ${lang === 'ar' ? 'font-cairo font-bold tracking-normal' : ''}`}>{tire.model_name}</h1>
          </div>

          <div className="mb-12">
            <h3 className={`text-[10px] uppercase tracking-widest text-ash mb-6 ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("tire.specifications")}</h3>
            
            {tire.specs && Object.keys(tire.specs).length > 0 ? (
              <div className="grid grid-cols-2 gap-4">
                {Object.entries(tire.specs).map(([key, value]) => {
                  // Check if the translation exists in our specs dictionary, otherwise fallback to the cleaned DB key
                  ...
                  return (
                    <div key={key} className="bg-carbon/50 border border-white/5 p-4 rounded-xl text-start">
                      <span className={`block text-[9px] uppercase tracking-wider text-ash mb-1 ${lang === 'ar' ? 'font-cairo font-bold' : ''}`}>
                        {displayKey}
                      </span>
                      <span className="block text-sm font-medium text-white">{String(value)}</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div className={`text-ash font-light text-sm ${lang === 'ar' ? 'font-cairo' : ''}`}>
                {t("tire.spec_notice")}
              </div>
            )}
          </div>
          ...
          {tire.homologatedBrands && tire.homologatedBrands.length > 0 && (
            <div className="mb-12">
              <h3 className={`text-[10px] uppercase tracking-widest text-ash mb-4 ${lang === 'ar' ? 'font-cairo' : ''}`}>{t("tire.homologated_for")}</h3>
              <div className="flex flex-wrap gap-2">
                {tire.homologatedBrands.map((brand) => (
                  <span key={brand.id} className="px-4 py-2 border border-white/10 rounded-full text-xs text-white/80 bg-white/5">{brand.name}</span>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-auto pt-8">
            <button onClick={() => router.push(`/order/vehicle?tire_id=${tire.id}`)} className={`w-full md:w-auto bg-crimson text-white px-12 py-5 rounded-xl uppercase tracking-widest text-xs font-bold hover:bg-white hover:text-obsidian transition-all duration-500 shadow-[0_0_30px_rgba(204,0,0,0.3)] flex items-center justify-center gap-4 ${lang === 'ar' ? 'font-cairo font-bold tracking-normal' : ''}`}>
              {t("tire.order_now")}
              <svg className={`w-4 h-4 ${lang === 'ar' ? 'rtl:-scale-x-100' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
            </button>
            <p className={`mt-4 text-center md:text-start text-[10px] text-ash tracking-widest uppercase ${lang === 'ar' ? 'font-cairo' : ''}`}>
              {t("tire.step_1")}
            </p>
          </div>
        </div>
